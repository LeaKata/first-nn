...
class ReLuNN:
    ...
    def training(self, x_train, y_train, repetitions):
        ...
        for rep in range(repetitions):
            ...
            for i in range(y_length):
                ...
                out_layer = self.relu_layer(second_layer, w_2, b_2)

                error = (out_layer - y)
                se = error ** 2
                self.training_error.append(np.sum(se) / 10)
                ...
                nb_2 = b_2 - self.alpha * ((2 / 10) * error)
                nw_2 = w_2 - self.alpha * ((2 / 10) * error.dot(second_layer.T))

                nb_1 = b_1 - self.alpha * ((2 / 10) * w_2.T.dot(error))
                nw_1 = w_1 - self.alpha * ((2 / 10) * w_2.T.dot(error.dot(first_layer.T)))

                nb_0 = b_0 - self.alpha * ((2 / 10) * (w_2.dot(w_1)).T.dot(error))
                nw_0 = w_0 - self.alpha * ((2 / 10) * (w_2.dot(w_1)).T.dot(error.dot(input_layer.T)))
    ...
    def evaluation(self, x_test, y_test):
        ...
        for i in range(len(y_test)):
                ...
                error = (out_layer - y)
                se = error ** 2
                self.evaluation_error.append(np.sum(se) / 10)
    ...
    def classify(self, input_image):
        ...
        out_layer = self.relu_layer(second_layer, self.w_2, self.b_2)
