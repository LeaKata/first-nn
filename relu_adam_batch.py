    ...
    def training(self, x_input, y_input, validation_fraction, batch_size, repetitions):
        ...
        for rep in range(repetitions):
            ...
            for batch in range(y_length // batch_size):
                gb_0, gb_1, gb_2, gw_0, gw_1, gw_2 = (0 for i in range(6))
                t += 1

                for index in range(batch_size):
                    progress += 1
                    i = (batch_size * batch) + index
                    y = np.zeros((10, 1))
                    y[y_train[i]] = 1
                    ...
                    if progress / progress_index * 100 % 2 == 0:
                        print('Training %', int(progress / progress_index * 100), ', Loss:', np.round(error, 5))
                    ...
                    gb_1 += i1 * (out_layer - y).T.dot(w_2).T
                    gw_1 += i1 * ((first_layer.dot((out_layer - y).T)).dot(w_2)).T

                    gb_0 += i0 * (out_layer - y).T.dot(w_2.dot(w_1 * i1)).T
                    gw_0 += i0 * ((input_layer.dot((out_layer - y).T)).dot(w_2.dot(w_1 * i1))).T
